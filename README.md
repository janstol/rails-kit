...
This is an **approximation**, not a replacement for `rails routes`. It understands `resources`/`resource`, standard or parenthesized `namespace` blocks, single-line static scopes with positional or `path:`/`module:`/`as:`/`controller:` context, single-line static `controller` blocks, `root`, recursively drawn files under `config/routes`, static block-defined route concerns, single-declaration inline namespace/member/collection blocks, single-line or comma-continued verb routes (`get`/`post`/`put`/`patch`/`delete`) and `match` routes with static `via:` methods, literal string redirects with an optional numeric `status:`, and static constant or constant-qualified zero-argument receiver mounts using hash-rocket or `at:` syntax, including nesting, `member`/`collection` blocks, explicit or inferred controller actions and helper names, resource `path`/`controller`/`as`/`param`/`concerns` options, and scalar, array, `%i[...]`, or `%w[...]` action filters. Generated resource helpers follow Rails inspector output, so only the first enabled route sharing a helper displays its prefix. Multiple `match` methods use Rails inspector-style text such as `GET|POST`, while `via: :all` has an empty verb. Mount entries also have an empty verb and represent only the mount point; receiver mounts have no inferred helper, internal engine routes are not expanded, and a custom runtime `engine_name` may differ from the statically inferred helper. Postfix-conditional mounts are included with warnings because their runtime conditions are not evaluated. Redirect endpoints use Rails inspector-style text such as `redirect(301, /new)`. Inline regex constraints whose keys match named path parameters use Rails inspector-style endpoint suffixes such as `{id: /[0-9]+/}`. Drawn files and concerns inherit the surrounding route scope; missing, unsafe, unreadable, or cyclic expansions produce warnings while parsing continues. Custom runtime `draw_paths`, callable or parameterized concerns, concern option propagation, multiline or dynamic scopes or controller blocks, executable, nested, or multi-statement inline blocks, multiline mount declarations, dynamic `match` declarations, receiver calls with arguments, dynamic, callable, block, or option-hash redirects, and request-level, object, block, resource-level, or non-comma-continued multiline constraints are not modeled. The parser does **not** expand routes drawn by gems (e.g. Devise). Unsupported constraints retain their approximate routes and produce warnings. Unsupported or partially modeled route DSL is reported with its source file and line on stderr while successfully parsed routes remain on stdout, including in JSON mode. Use it to get a quick answer for "what routes exist for X" when Rails can't boot or a fast answer is more valuable than a complete one — not as a source of truth for CI or production route audits.
