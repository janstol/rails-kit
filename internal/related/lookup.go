...
func resolvePathModelName(root string, cfg config.Config, input string, p *pluralize.Pluralizer) (string, error) {
	...
	rules := []pathRule{
		...
		{prefix: cfg.HelpersPath, resolve: func(rel string) []string {
			return ResourceCandidates(rel, "_helper.rb", p)
		}},
		{prefix: cfg.DecoratorsPath, resolve: func(rel string) []string {
			return ResourceCandidates(rel, "_decorator.rb", p)
		}},
		{prefix: cfg.PresentersPath, resolve: func(rel string) []string {
			return ResourceCandidates(rel, "_presenter.rb", p)
		}},
