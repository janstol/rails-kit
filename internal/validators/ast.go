...
// Parse reads a validator file, parses it with Prism, and returns its
...
// still returned.
func Parse(validatorPath, railsRoot, validatorsPath string) (*Summary, error) {
	p, err := astutil.ParseFile(validatorPath)
	if err != nil {
		return nil, err
	}

	s := &Summary{}
	s.RelPath, s.ClassName = astutil.SummaryPath(validatorPath, railsRoot, validatorsPath)
	...
	w := validatorWalker{src: p.Src, summary: s}
...
// rendered as a signature -- validate/validate_each's parameters are part of
// what distinguishes an EachValidator from a Validator, same reasoning as
// helpers, decorators, formers, and presenters.
func (w *validatorWalker) handleDef(def *parser.DefNode, visibility string, singleton bool) {
	if singleton {
...
// class-level call (validates, attr_reader, delegate, and anything a custom
// validator implementation makes) as a Macros catch-all. SkipVisibilityCalls
// keeps bare private/protected/public switches and the `private :a, :b`
// symbol-list form from leaking into Macros as noise.
func (w *validatorWalker) handleCall(call *parser.CallNode) {
...
// filter, same reasoning as services, helpers, decorators, formers, and
// presenters.
func (w *validatorWalker) handleInclude(args []parser.Node) {
...
// `  NAME = value`, collapsing any whitespace in the value expression. This
// is where a validator's actual rule usually lives -- a format regexp or an
// allowed-value list.
func (w *validatorWalker) handleConstant(n *parser.ConstantWriteNode) {
	if n.Value == nil {
		return
	}
...
func (w *validatorWalker) renderCall(call *parser.CallNode) string {
	entry := "  " + call.Name
	argSrc := ""
