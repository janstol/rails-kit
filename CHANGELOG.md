...
- Explicit `.rb` path resolution (`controllers`, `mailers`, `jobs`, `services`, `datagrids`, `helpers`, `decorators`, `formers`, `presenters`, `validators`, `model`) no longer lets a traversing relative path escape the domain directory by way of its Rails-root-relative fallback -- e.g. `rails-kit --root <app> services ../models/user.rb` used to report `app/models/user.rb` as a service when run from outside the app, while an absolute path to that same file was correctly rejected; all three interpretations of an explicit path are now validated against the domain directory the same way, so the answer no longer depends on the caller's working directory. The containment check itself is also narrower now, comparing path components instead of a raw string prefix, so a legitimate subdirectory whose name happens to start with two dots (e.g. `app/jobs/..backup/`) is no longer rejected as an escape attempt.
